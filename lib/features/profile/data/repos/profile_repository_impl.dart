...
class ProfileRepositoryImpl implements ProfileRepository {
  ...
  @override
  Future<Either<ApiErrorModel, bool>> updateProfile(ProfileEntity profile) async {
    try {
      if (await _isConnected()) {
  ...
  @override
  Future<Either<ApiErrorModel, UserOrderEntity>> getOrderDetails(String orderId) async {
    try {
      if (await _isConnected()) {
        ...
        return localResult.fold(
          ...
          (cachedOrders) {
            if (cachedOrders != null && cachedOrders.isNotEmpty) {
              final order = cachedOrders.where((o) => o.id == orderId).firstOrNull;
              if (order != null) {
                return Right(order);
              }
            }
  ...
  @override
  Future<Either<ApiErrorModel, List<LibraryItemEntity>>> getLibraryItems() async {
    try {
      // محاولة جلب البيانات من الخادم أولاً
      if (await _isConnected()) {
  ...
  @override
  Future<Either<ApiErrorModel, LibraryItemEntity>> getLibraryItemDetails(String itemId) async {
    try {
      if (await _isConnected()) {
        final remoteResult = await remoteDataSource.getLibraryItemDetails(itemId);
