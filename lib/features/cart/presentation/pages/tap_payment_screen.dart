...
class _TapPaymentScreenState extends State<TapPaymentScreen> {
  ...
  bool _isLoading = true;
  String? _errorMessage;
  bool _hasDataToInject = false; // متغير لتتبع وجود بيانات للحقن
  final bool _dataInjected = false; // متغير لتتبع حالة حقن البيانات
  bool _dialogShown = false; // متغير لتتبع عرض الـ dialog لأول مرة
  ...
  void initState() {
    ...
    // التحقق من وجود بيانات للحقن
    _hasDataToInject =
        (widget.cartData != null && widget.cartData!.isNotEmpty) ||
        (widget.customerData != null && widget.customerData!.isNotEmpty);
  ...
  void _refreshWebView() {
    ...
    setState(() {
      _isLoading = true;
      _errorMessage = null;
      // لا نعيد تعيين _dialogShown حتى لا يظهر الـ dialog مرة أخرى
    });
  }
            ...
            NavigationDelegate(
              ...
              onPageFinished: (String url) async {
                ...
                if (_hasDataToInject) {
                  ...
                  // عرض dialog التحديث لأول مرة فقط
                  if (!_dialogShown && mounted) {
                    _dialogShown = true; // تعيين أن الـ dialog تم عرضه
