  ...
  Future<void> registerWithEmailAndPassword() async {
    try {
      ...
      if (!email.contains('@') || !email.contains('.')) {
        emit(RegisterError(errorMessage: LocaleKeys.invalid_email_address.tr()));
        return;
      }
      
      // Password validation
      if (password.length < 6) {
        emit(RegisterError(errorMessage: LocaleKeys.password_must_be_at_least_6_characters_long.tr()));
        return;
      }
      ...
      emit(RegisterError(errorMessage: LocaleKeys.something_went_wrong_please_try_again.tr()));
    }
  }
